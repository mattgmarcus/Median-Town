...
def getDeptUrl(pair):
    try:
        if 2 == len(pair):
            ...
            print("Invalid pair length:", len(pair))
            return None
    except Exception as e:
        print("Error constructing URL:", str(e))
        return None
...
def getDescription(pair):
    ...
    dept_page = urllib.request.urlopen(dept_url).read().decode('utf-8')
    ...
    for link in course_links:
        course_page = BeautifulSoup(urllib.request.urlopen(root_url + link), 'html.parser')

        course = title = description = instructor = distrib = offered = None

        from bs4 import Tag
        if course_page.h1 and isinstance(course_page.h1, Tag):
            span = course_page.h1.find('span')
            if span:
                course = span.get_text()
                if len(course_page.h1.contents) > 2:
                    content = course_page.h1.contents[2]
                    if isinstance(content, Tag):
                        title = " ".join(content.get_text().split())

        desc_elem = course_page.find(class_="desc")
        if desc_elem:
            description = desc_elem.get_text().strip()

        def get_element_text(elem, index=1):
            if isinstance(elem, Tag) and len(elem.contents) > index:
                content = elem.contents[index]
                if isinstance(content, Tag):
                    return content.get_text()
            return None

        instr_elem = course_page.find(id="instructor")
        if instr_elem:
            instructor = get_element_text(instr_elem)

        distrib_elem = course_page.find(id="distribution")
        if distrib_elem:
            distrib = get_element_text(distrib_elem)

        offered_elem = course_page.find(id="offered")
        if offered_elem:
            offered = get_element_text(offered_elem)
